...
const Calendar = () => {
    ...
    const addDate = (day, month, year) => {
        ...
        if (dateArray.includes(dateToAdd)){
            setDateArray(oldValues => {
                return oldValues.filter(item => item !== dateToAdd)
              }
            );
        }
        else {
            setDateArray(oldArray => [...oldArray, dateToAdd]);
        };
    };
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 1; i <= totalDaysInMonth; i++) {
        if (dateArray.includes([i,month,year].toString())) {
            days.push(
                <DayCalendar 
                key={`current-${i}`} 
                text={i}
